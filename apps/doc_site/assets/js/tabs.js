...
export default {
  mounted() {
    this.el.addEventListener("switch-tabs", (event) => {
      ...
      const active_tab = this.el.querySelector(
        `[key="${tab_prefix}${active_key}"]`
      );
      ...
      let contents = Array.from(
        this.el.querySelector(".tabs-content").children
      );
    ...
    this.el.dispatchEvent(
      new CustomEvent("switch-tabs", { detail: { active_key } })
    );
  },
};
