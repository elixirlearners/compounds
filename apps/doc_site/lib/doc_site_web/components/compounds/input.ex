defmodule Compounds.Input do
  ...
  attr :input_id, :string, required: false
  ...
  def input(assigns) do
    assigns =
      assign(assigns,
        ...
        id: Compounds.Id.generate("input"),
        input_id: Map.get(assigns, :input_id, Compounds.Id.generate("input-field"))
      )
  ...
  def label_left(assigns) do
    assigns = assign(assigns, style: "rounded-tr-0 rounded-br-0")

    ~H"""
    <.input_label class={Tails.classes([@style, @class])} right?={false}>
      <%= render_slot(@inner_block) %>
    </.input_label>
    """
  end
  ...
  slot :inner_block
  attr :class, :string, default: nil
  attr :right?, :boolean, default: true
  ...
  #   ~H"""
  #   <div phx-click={JS.set_attribute({"value", ""}, to: "#" <> @input_id)} id="clear-svg" class={Tails.classes([@style, @class])}>
  #     <svg
  #       viewBox="0 0 24 24"
  #       stroke="currentColor"
  #       strokeWidth="1.5"
